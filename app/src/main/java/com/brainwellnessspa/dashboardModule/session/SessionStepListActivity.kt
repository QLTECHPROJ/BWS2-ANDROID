...
import com.brainwellnessspa.databinding.ActivitySessionStepListBinding
...
class SessionStepListActivity : AppCompatActivity() {
    lateinit var binding: ActivitySessionStepListBinding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = DataBindingUtil.setContentView(this, R.layout.activity_session_step_list)
        act = this@SessionStepListActivity
        ctx = this@SessionStepListActivity
...
//                                                        act.finish()
    ...
    class SessionDetailAdapter(var binding: ActivitySessionStepListBinding, var catName: List<SessionStepListModel.ResponseData.Data>?, val act: Activity, var userId: String?, var sessionId: String?, val response: SessionStepListModel.ResponseData, val ctx: Context) : RecyclerView.Adapter<SessionDetailAdapter.MyViewHolder>() {
        ...
        override fun onBindViewHolder(holder: MyViewHolder, position: Int) {
            ...
            holder.bindingAdapter.llBorder.setOnClickListener {
                when {
                    response.shouldFillProfileFormOne.equals("1") -> {
                        val i = Intent(ctx, EEPProfileStepOneActivity::class.java)
                        i.flags = Intent.FLAG_ACTIVITY_SINGLE_TOP
                        act.startActivity(i)
                    }response.shouldFillProfileFormTwo.equals("1") -> {
                        val i = Intent(ctx, EEPProfileStepTwoActivity::class.java)
                        i.flags = Intent.FLAG_ACTIVITY_SINGLE_TOP
                        act.startActivity(i)
                    }
                    response.shouldFillProfileFormThree.equals("1") -> {
                        val i = Intent(ctx, EEPProfileStepThreeActivity::class.java)
                        ...
                        when (db.stepId) {
                            ...
                            "2" -> {
                                /* TODO IMPORTANT FOR FORAM
                                        *  TODO Progress report
                                        *   nextForm key use open next screen */
                                val sd = SessionStepListActivity()
                            ...
                            if (listModel1?.responseCode.equals(act.getString(R.string.ResponseCodesuccess), ignoreCase = true)) {
                                ...
                                if (response != null) {
                                    if (response.questionStatus.equals("0")) {
                                        val i = Intent(act, SessionBeforeAfterQustionActivity::class.java)
                                        i.putExtra("SessionId", sessionId)
