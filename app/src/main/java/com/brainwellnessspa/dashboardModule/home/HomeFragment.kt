...
class HomeFragment : Fragment() {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View {
        ...
        ctx = requireActivity()
        act = requireActivity()
        /* Get UserId, and MAin Account Id from share pref*/
        ...
        binding.llBottomView.setOnClickListener {
            if (isNetworkConnected(activity)) {
                val layoutBinding: UserListCustomLayoutBinding = DataBindingUtil.inflate(LayoutInflater.from(act), R.layout.user_list_custom_layout, null, false)
    ...
    fun prepareHomeData() {
        /* Get firebase token form share pref*/
            ...
            listCall.enqueue(object : Callback<HomeScreenModel?> {
                ...
                override fun onResponse(call: Call<HomeScreenModel?>, response: Response<HomeScreenModel?>) {
                    try {
                        ...
                        getPastIndexScore(homelistModel.responseData!!, binding.barChart, act)
                ...
                if (AudioPlayerFlag.equals("Downloadlist", ignoreCase = true) && MyPlaylist.equals(playlistID, ignoreCase = true)) {
                    if (isDisclaimer == 1) {
                        ...
                        if (player != null) {
                            ...
                            callPlayerSuggested(position, view, listModel, ctx, act!!, playlistID, playlistName, true)
                        }
                    }
                } else {
    ...
    inner class UserListAdapter(private val model: AddedUserListModel.ResponseData, val mBottomSheetDialog: BottomSheetDialog) : RecyclerView.Adapter<UserListAdapter.MyViewHolder>() {
        var selectedItem = -1
        var pos = 0
        override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): MyViewHolder {
        ...
        override fun onBindViewHolder(holder: MyViewHolder, position: Int) {
            ...
            if (modelList[position].image.equals("", ignoreCase = true)) {
                ...
                Glide.with(act).load(modelList[position].image).thumbnail(0.10f).apply(RequestOptions.bitmapTransform(RoundedCorners(126))).into(holder.bind.ivProfileImage)
            }
                                            ...
                                            when {
                                                listModel.responseCode.equals(getString(R.string.ResponseCodesuccess), ignoreCase = true) -> {
                                                    ...
                                                    if (responseData!!.isProfileCompleted.equals("0", ignoreCase = true)) {
                                                        ...
                                                        act.startActivity(intent)
                                                        act.finish()
                                                    } else if (responseData.avgSleepTime.equals("", ignoreCase = true)) {
                                                        val intent = Intent(activity, SleepTimeActivity::class.java)
                                                        act.startActivity(intent)
                                                        act.finish()
                                                    } else if (responseData.isProfileCompleted.equals("1", ignoreCase = true) && responseData.isAssessmentCompleted.equals("1", ignoreCase = true)) {
                                                    ...
                                                    editor.apply()
                                                    val sharedd = act.getSharedPreferences(CONSTANTS.RecommendedCatMain, Context.MODE_PRIVATE)
                                                    val editord = sharedd.edit()
                                                    editord.putString(CONSTANTS.PREFE_ACCESS_SLEEPTIME, listModel.responseData!!.avgSleepTime)
                                                    ...
                                                    editord.putString(CONSTANTS.selectedCategoriesTitle, gson.toJson(selectedCategoriesTitle)) //Friend
                                                    editord.putString(CONSTANTS.selectedCategoriesName, gson.toJson(selectedCategoriesName)) //Friend
                                                    editord.apply()
                                                    prepareHomeData()
                                                    //                                                    showToast(
