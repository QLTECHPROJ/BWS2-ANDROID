...
public class ProfileFragment extends Fragment {
    ...
    String USERID, CoUserID, UserEmail, userImage, DeviceID, DeviceType, UserName, profilePicPath = "", userMobile, isProfileCompleted, isAssessmentCompleted, indexScore, scoreLevel, avgSleepTime;
    //    areaOfFocus
    ...
    public View onCreateView(@NotNull LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        SharedPreferences shared = requireActivity().getSharedPreferences(CONSTANTS.RecommendedCatMain, Context.MODE_PRIVATE);
        ...
        if (BWSApplication.isNetworkConnected(getActivity())) {
            ...
            if (UserName.equalsIgnoreCase("") || UserName.equalsIgnoreCase(" ") || UserName == null) {
    ...
    private void selectImage() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M && Build.VERSION.SDK_INT <= Build.VERSION_CODES.Q) {
            if (ContextCompat.checkSelfPermission(requireActivity(), Manifest.permission.WRITE_EXTERNAL_STORAGE) == PackageManager.PERMISSION_GRANTED && ContextCompat.checkSelfPermission(requireActivity(), Manifest.permission.READ_EXTERNAL_STORAGE) == PackageManager.PERMISSION_GRANTED && ContextCompat.checkSelfPermission(requireActivity(), Manifest.permission.CAMERA) == PackageManager.PERMISSION_GRANTED) {
                ...
                mRequestPermissionHandler.requestPermission(requireActivity(), new String[]{Manifest.permission.WRITE_EXTERNAL_STORAGE, Manifest.permission.READ_EXTERNAL_STORAGE, Manifest.permission.CAMERA}, 123, new RequestPermissionHandler.RequestPermissionListener() {
                    @Override
            ...
            if (ContextCompat.checkSelfPermission(requireActivity(), Manifest.permission.READ_EXTERNAL_STORAGE) == PackageManager.PERMISSION_GRANTED && ContextCompat.checkSelfPermission(requireActivity(), Manifest.permission.CAMERA) == PackageManager.PERMISSION_GRANTED) {
                ...
                if (ContextCompat.checkSelfPermission(requireActivity(), Manifest.permission.READ_EXTERNAL_STORAGE) != PackageManager.PERMISSION_GRANTED && ContextCompat.checkSelfPermission(requireActivity(), Manifest.permission.CAMERA) != PackageManager.PERMISSION_GRANTED) {
                    mRequestPermissionHandler.requestPermission(requireActivity(), new String[]{Manifest.permission.CAMERA, Manifest.permission.READ_EXTERNAL_STORAGE}, 123, new RequestPermissionHandler.RequestPermissionListener() {
                        @Override
                        public void onSuccess() {
                            callProfilePathSet();
                        }
    ...
    private void callCamaraPermission() {
        ...
        building.setPositiveButton(getString(R.string.Settings), (dialogs, id1) -> {
    ...
    private void callReadPermission() {
        ...
        buildable.setPositiveButton(getString(R.string.Settings), (dialogs, id1) -> {
            Intent intent = new Intent(Settings.ACTION_APPLICATION_DETAILS_SETTINGS);
            Uri uri = Uri.fromParts("package", requireActivity().getPackageName(), null);
            intent.setData(uri);
            startActivity(intent);
        ...
        if (requestCode == CONTENT_REQUEST && resultCode == Activity.RESULT_OK) {
            try {
                ...
                if (BWSApplication.isNetworkConnected(requireActivity())) {
                    ...
                    APIClientProfile.getApiService().getAddProfiles(CoUserID, typedFile, new retrofit.Callback<AddProfileModel>() {
                        @Override
    ...
    private void setProfilePic(String profilePicPath) {
        Glide.with(requireActivity()).load(profilePicPath).thumbnail(0.10f).apply(RequestOptions.bitmapTransform(new RoundedCorners(126))).into(binding.civProfile);
    }
