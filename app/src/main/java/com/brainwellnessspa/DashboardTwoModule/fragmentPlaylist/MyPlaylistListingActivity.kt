...
//            handler2.removeCallbacks(UpdateSongTime2);
                            ...
                            i.putExtra("PlaylistImage", listModel.responseData!!.playlistImage!!)
...
//                            .registerReceiver(listener, IntentFilter("play_pause_Action"))
    ...
    class PlayListsAdpater(var listModel: List<PlaylistDetailsModel.ResponseData.PlaylistSong>, var ctx: Context, var CoUserID: String?, var created: String?, var binding: ActivityMyPlaylistListingBinding, var activity: Activity, var startDragListener: StartDragListener, var PlaylistID: String?, var PlaylistName: String?) : RecyclerView.Adapter<PlayListsAdpater.MyViewHolder>(), ItemTouchHelperContract {
        ...
        override fun onBindViewHolder(holder: MyViewHolder, position: Int) {
            ...
            if (AudioPlayerFlag.equals("SubPlayList", ignoreCase = true) && MyPlaylist.equals(PlaylistID, ignoreCase = true)) {
            ...
            holder.binding.llMainLayout.setOnClickListener {
                MyPlaylistListingActivity().callMainPlayer(position, "Created", listModel, ctx, activity, listModel[0].playlistID!!,created)
            }
...
//                MyPlaylistListingActivity().callMainPlayer(position, "", listModel, ctx, activity)
                if (isPlayPlaylist == 1) {
                    ...
                    MyPlaylistListingActivity().callMainPlayer(0, "Created", listModel, ctx, activity, listModel[0].playlistID!!, created)
                    binding.llPlay.visibility = View.GONE
                    binding.llPause.visibility = View.VISIBLE
                }
...
//                                            player.seekTo(pos, 0);
                                            ...
                                            MyPlaylistListingActivity().callMainPlayer(PlayerPosition, "Created", listModel, ctx, activity, listModel[0].playlistID!!, created)
                                        }
                                    }
...
//                    }
    ...
    class PlayListsAdpater2(var listModel: List<PlaylistDetailsModel.ResponseData.PlaylistSong>, var ctx: Context, var CoUserID: String?, var created: String?, var binding: ActivityMyPlaylistListingBinding, var activity: Activity, var PlaylistID: String?, var PlaylistName: String?, var MyDownloads: String?) : RecyclerView.Adapter<PlayListsAdpater2.MyViewHolder>(), Filterable {
...
//            binding.tvSearch.setVisibility(View.GONE)
            ...
            holder.binding.llMainLayout.setOnClickListener {
                MyPlaylistListingActivity().callMainPlayer(position, "", listFilterData, ctx, activity, listModel[0].playlistID!!, created)
            }
            binding.llPlayPause.setOnClickListener {
//                MyPlaylistListingActivity().callMainPlayer(position, "", listModel, ctx, activity)
                if (isPlayPlaylist == 1) {
                    ...
                    MyPlaylistListingActivity().callMainPlayer(0, "", listModel, ctx, activity, listModel[0].playlistID!!, created)
                    binding.llPlay.visibility = View.GONE
                    binding.llPause.visibility = View.VISIBLE
                }
                notifyDataSetChanged()
            ...
            /* binding.llDelete.setOnClickListener { v ->
                 ...
                 if (audioPlay && AudioFlag.equals("Downloadlist", ignoreCase = true) && pID.equals(PlaylistName, ignoreCase = true)) {
                     ...
                     Btn.setOnClickListener { views: View? ->
                         ...
                         val fragmentManager1: FragmentManager = (ctx as FragmentManager).getSupportFragmentManager()
...
//                        Log.e("search", SearchFlag)
    ...
    fun callMainPlayer(position: Int, view: String?, listModel: List<PlaylistDetailsModel.ResponseData.PlaylistSong>, ctx: Context, act: Activity, playlistID: String, created: String?) {
        val shared1 = ctx.getSharedPreferences(CONSTANTS.PREF_KEY_PLAYER, MODE_PRIVATE)
        val AudioPlayerFlag = shared1.getString(CONSTANTS.PREF_KEY_AudioPlayerFlag, "0")
        val MyPlaylist = shared1.getString(CONSTANTS.PREF_KEY_PayerPlaylistId, "")
        val PlayFrom = shared1.getString(CONSTANTS.PREF_KEY_PlayFrom, "")
    ...
    private fun callPlayer(position: Int, view: String?, listModel: List<PlaylistDetailsModel.ResponseData.PlaylistSong>, ctx: Context, act: Activity, playlistID: String, created: String?) {
    ...
    private fun getMediaByPer(PlaylistId: String, totalAudio: Int) {
        try {
            DB!!.taskDao().getCountDownloadProgress1("Complete", PlaylistId).observe(this, { countx: List<DownloadPlaylistDetails?> ->
                count = countx.size
                ...
                if (count <= totalAudio) {
                    if (count == totalAudio) {
                        ...
                        DB!!.taskDao().getCountDownloadProgress1("Complete", PlaylistId).removeObserver { cs: List<DownloadPlaylistDetails?>? -> }
                    } else {
...
//                }
    ...
    private fun callDownload(id: String, audioFile: String, Name: String, playlistSongs: ArrayList<PlaylistDetailsModel.ResponseData.PlaylistSong>, position: Int, llDownload: RelativeLayout, ivDownloads: ImageView) {
        if (id.isEmpty() && Name.isEmpty() && audioFile.isEmpty()) {
                ...
                for (y in downloadAudioDetailsList.indices) {
                    if (playlistSongs2.size == 0) {
                        ...
                        for (x in playlistSongs2.indices) {
                            if (playlistSongs2.size != 0) {
                                if (playlistSongs2[x].audioFile.equals(downloadAudioDetailsList[y]!!.audioFile, ignoreCase = true)) {
            ...
            if (downloadAudioDetailsList.size != 0) {
                for (i in downloadAudioDetailsList.indices) {
                    if (downloadAudioDetailsList[i]!!.audioFile.equals(audioFile, ignoreCase = true)) {
                        downloadOrNot = false
                        break
            ...
            if (AudioPlayerFlag.equals("DownloadListAudio", ignoreCase = true)) {
                ...
                val sharedd: SharedPreferences = getSharedPreferences(CONSTANTS.PREF_KEY_AUDIO, MODE_PRIVATE)
...
//        BWSApplication.addToSegment("Playlist Download Started", p, CONSTANTS.track)
        for (i in playlistSongs.indices) {
            ...
            if (downloadAudioDetailsList.size != 0) {
                for (y in downloadAudioDetailsList.indices) {
                    if (playlistSongs[i].audioFile.equals(downloadAudioDetailsList[y]!!.audioFile, ignoreCase = true)) {
...
//        savePlaylist();
    ...
    private fun SaveMedia(playlistSongs: ArrayList<PlaylistDetailsModel.ResponseData.PlaylistSong>, i: Int, llDownload: RelativeLayout, ivDownloads: ImageView, progress: Int) {
        val downloadAudioDetails = DownloadAudioDetails()
        downloadAudioDetails.id = playlistSongs[i].id
        downloadAudioDetails.name = playlistSongs[i].name
        downloadAudioDetails.audioFile = playlistSongs[i].audioFile
    ...
    fun GetMedia(url: String?, ctx: Context?, download: String, llDownload: RelativeLayout, ivDownloads: ImageView) {
        DatabaseClient
                .getInstance(this)
                .getaudioDatabase()
                .taskDao()
...
//        playlistWiseAudioDetails = new ArrayList<>();
            DB!!.taskDao().getAllAudioByPlaylist1(PlaylistID).observe(this, { audioList: List<DownloadAudioDetails> ->
                if (MyDownloads.equals("1", ignoreCase = true)) {
                    if (downloadPlaylistDetailsList.size != 0) {
...
//                        listModel.isReminder = downloadPlaylistDetailsList[0]!!.isReminder
    ...
    fun GetPlaylistMedia(playlistID: String) {
        DB!!.taskDao().getAllAudioByPlaylist1(playlistID).observe(this, { audioList: List<DownloadAudioDetails> ->
            ...
            if (audioList.size != 0) {
                GetSingleMedia(audioList[0].audioFile, getApplicationContext(), playlistID, audioList, 0)
            }
        })
    }
    ...
    fun GetSingleMedia(AudioFile: String?, ctx: Context, playlistID: String?, audioList: List<DownloadAudioDetails>, i: Int) {
        DB!!.taskDao().getLastIdByuId1(AudioFile).observe(this, { audioList1: List<DownloadAudioDetails> ->
            try {
                ...
                if (i < audioList.size - 1) {
                    GetSingleMedia(audioList[i + 1].audioFile, ctx.applicationContext, playlistID, audioList, i + 1)
