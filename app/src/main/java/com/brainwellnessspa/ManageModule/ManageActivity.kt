            ...
            override fun onProgressChanged(seekBar: SeekBar, progresValue: Int, fromUser: Boolean) {
                if (progresValue >= 1) {
                    ...
                    for (i1 in listModelGloble!!.responseData!!.plan!!.indices) {
                        if (listModelGloble!!.responseData!!.plan!![i1].profileCount!!.equals(value.toString())) {
...
//            holder.binding.videoView.start()
    ...
    class PlanListAdapter(var listModelList: List<PlanlistInappModel.ResponseData.Plan>, var ctx: Context, var i: Intent) :
        ...
        override fun onBindViewHolder(holder: MyViewHolder, position: Int) {
            ...
            if (row_index == position) {
                ...
                if (listModelList[position].recommendedFlag.equals("1", ignoreCase = true) && pos == 0) {
                    ...
                    holder.binding.llPlanMain.background = ctx.resources.getDrawable(R.drawable.light_gray_round_cornors)
                    holder.binding.tvTilte.setTextColor(ctx.resources.getColor(R.color.black))
                    holder.binding.tvContent.setTextColor(ctx.resources.getColor(R.color.black))
                    holder.binding.tvAmount.setTextColor(ctx.resources.getColor(R.color.black))
                }
            }

        }

        private fun ChangeFunction(holder: PlanListAdapter.MyViewHolder, listModelList: List<PlanlistInappModel.ResponseData.Plan>, position: Int) {
            holder.binding.llPlanMain.background = ctx.resources.getDrawable(R.drawable.light_sky_round_cornors)
            holder.binding.tvTilte.setTextColor(ctx.resources.getColor(R.color.black))
            holder.binding.tvContent.setTextColor(ctx.resources.getColor(R.color.black))
            holder.binding.tvAmount.setTextColor(ctx.resources.getColor(R.color.black))
            val gson = Gson()
